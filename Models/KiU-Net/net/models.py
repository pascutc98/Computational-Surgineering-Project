...
class segnet(nn.Module):
    def __init__(self, training):
        ...
        self.decoder3 = nn.Conv3d(128, 64, 3, stride=1, padding=1)  # b, 1, 28, 28
        self.decoder4 = nn.Conv3d(64, 32, 3, stride=1, padding=1)
        self.decoder5 = nn.Conv3d(32, 2, 3, stride=1, padding=1) # self.decoder5 =   nn.Conv3d(32, 2, 3, stride=1, padding=1)
                                #(32, 3...
        self.map4 = nn.Sequential(
            nn.Conv3d(3, 2, 1, 1), #nn.Conv3d(2, 1, 1, 1) / nn.Conv3d(3, 3, 1, 1)
        ...
        # 128*128 尺度下的映射
        self.map3 = nn.Sequential(
            nn.Conv3d(64, 2, 1, 1), #nn.Conv3d(64, 1, 1, 1) / nn.Conv3d(64, 3, 1, 1)
        ...
        # 64*64 尺度下的映射
        self.map2 = nn.Sequential(
            nn.Conv3d(128, 2, 1, 1), #nn.Conv3d(128, 1, 1, 1) / nn.Conv3d(128, 3, 1, 1)
        ...
        # 32*32 尺度下的映射
        self.map1 = nn.Sequential(
            nn.Conv3d(256, 2, 1, 1), #nn.Conv3d(256, 1, 1, 1) / nn.Conv3d(256, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        out = F.relu(F.interpolate(self.decoder1(out),scale_factor=(2,2,2),mode ='trilinear'))
        # print(out.shape,t4.shape)
        out = torch.add(out,t4) # out = torch.add(F.pad(out,[0,0,0,0,0,1]),t4)
        output1 = self.map1(out)
        out = F.relu(F.interpolate(self.decoder2(out),scale_factor=(2,2,2),mode ='trilinear'))
        # out = torch.add(out,t3)
        output2 = self.map2(out)
...
class kiunet_org(nn.Module):
    ...
    def __init__(self,training):
        ...
        self.final = nn.Conv3d(8,2,1,stride=1,padding=0) #self.final = nn.Conv3d(8,3,1,stride=1,padding=0)
        self.fin = nn.Conv3d(2,2,1,stride=1,padding=0) #self.fin = nn.Conv3d(3,3,1,stride=1,padding=0)

        self.map4 = nn.Sequential(
            nn.Conv3d(32, 2, 1, 1), #nn.Conv3d(32, 3, 1, 1),
            nn.Upsample(scale_factor=(8, 8, 8), mode='trilinear'), #nn.Upsample(scale_factor=(4, 16, 16), mode='trilinear')
            # nn.Sikgmoid() # No sigmoid for cross-entropy loss
        )

        # 128*128 尺度下的映射
        self.map3 = nn.Sequential(
            nn.Conv3d(16, 2, 1, 1), #nn.Conv3d(16, 3, 1, 1)
            nn.Upsample(scale_factor=(8, 4, 4), mode='trilinear'), #nn.Upsample(scale_factor=(4, 8, 8), mode='trilinear')
            # nn.Sigmoid() # No sigmoid for cross-entropy loss
        )

        # 64*64 尺度下的映射
        self.map2 = nn.Sequential(
            nn.Conv3d(8, 2, 1, 1), #nn.Conv3d(8, 3, 1, 1)
            nn.Upsample(scale_factor=(4, 2, 2), mode='trilinear'), #nn.Upsample(scale_factor=(4, 4, 4), mode='trilinear')
            # nn.Sigmoid() # No sigmoid for cross-entropy loss
        )

        # 32*32 尺度下的映射
        self.map1 = nn.Sequential(
            nn.Conv3d(256, 2, 1, 1), #nn.Conv3d(256, 3, 1, 1)
            nn.Upsample(scale_factor=(16, 4, 4), mode='trilinear'), # nn.Upsample(scale_factor=(16, 32, 32), mode='trilinear')
            # nn.Sigmoid() # No sigmoid for cross-entropy loss
        )
    ...
    def forward(self, x):
        ...
        out1 = torch.add(out1,F.interpolate(F.relu(self.inte3_2bn(self.intere3_2(tmp))),scale_factor=(4,16,16),mode ='trilinear')) # torch.add(out1,F.interpolate(F.relu(self.inte3_2bn(self.intere3_2(tmp))),scale_factor=(2,16,16),mode ='trilinear'))
        ...
        out1 = torch.add(out1,F.interpolate(F.relu(self.intd1_2bn(self.interd1_2(tmp))),scale_factor=(1,4,4),mode ='trilinear')) # out1 = torch.add(out1,F.interpolate(F.relu(self.intd1_2bn(self.interd1_2(tmp))),scale_factor=(0.5,4,4),mode ='trilinear'))
        # print(out.shape)
        output1 = self.map4(out)
        out = torch.add(out,u2)  #skip conn
        out1 = F.interpolate(out1,scale_factor=(2,1,1),mode ='trilinear') # Added to match the depth 
        ...
        out = F.interpolate(out, scale_factor=(2,1,1), mode='trilinear')
        ...
        output4 = F.interpolate(self.fin(out),scale_factor=(4,2,2),mode ='trilinear') #        output4 = F.interpolate(self.fin(out),scale_factor=(4,4,4),mode ='trilinear')
        # print(out.shape)
        # out = self.soft(out)
        # print(output1.shape,output2.shape,output3.shape,output4.shape)
        if self.training is True:
